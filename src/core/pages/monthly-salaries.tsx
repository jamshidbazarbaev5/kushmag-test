...
export default function MonthlySalariesPage() {
  ...
  const { mutate: createMonthlySalary, isPending: isCreating } =
    useCreateMonthlySalary();
  const { mutate: updateMonthlySalary, isPending: isUpdating } =
    useUpdateMonthlySalary();
  ...
  const calculateOrderPercentageSalary = (
        ...
        onSuccess: () => {
          toast.success(
            t("messages.success.created", {
              item: t("navigation.monthly_salaries"),
            }),
          );
          handleCancel();
        },
  ...
  const handleFieldChange = (
    ...
    if (field === "month" && value !== undefined && value !== null) {
    ...
    if (field === "user") {
      const selectedUser = users.find(
        (user: User) => user.id === Number(value),
      );
      if (selectedUser) {
        ...
        updatedData.order_percentage_salary = calculateOrderPercentageSalary(
          selectedUser.order_percentage || 1,
          ordersTotal,
        );
      }
    }
  ...
  const handleCreateUser = (data: User) => {
    createUser(data, {
      onSuccess: (newUser: User) => {
        ...
        handleFieldChange("user", newUser.id || 0);
      },
      onError: () =>
        toast.error(
  ...
  const renderEditableCell = (
    ...
    if (type === "select") {
      return (
        <Select
          ...
          onValueChange={(value) =>
            handleFieldChange(field, field === "user" ? Number(value) : value)
          }
        >
    ...
    return (
      <Input
        ...
        disabled={
          field === "total_salary" || field === "order_percentage_salary"
        }
      />
    );
  };
                  ...
                  <TableRow key={salary.id}>
                    ...
                    <TableCell>
                      {editingRowId === salary.id
                        ...
                        : salary.user_details
                          ? `${salary.user_details.full_name} (${salary.user_details.username})`
                          : users.find((user: User) => user.id === salary.user)
