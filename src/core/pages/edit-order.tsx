...
export default function CreateOrderPage() {
    ...
    calculateOrder(calculationData, {
      onSuccess: (response) => {
        ...
        if (discountAmount && discountAmount.trim() !== "") {
          ...
          discountAmountValue = parseFloat(
            discountAmount.replace(/,/g, ".") || "0",
          );
  ...
  const onSubmit = async (data: any) => {
    ...
    const discountPercentageForAPI =
      total_sum > 0 ? (calculatedDiscountAmount / total_sum) * 100 : 0;
...
function StepTwo({
  ...
  const handleEditDoor = (index: number) => {
    ...
    // Ensure the door has at least 2 extensions
    const extensions = door.extensions || [];
    ...
    // Ensure the door has at least 2 casings
    const casings = door.casings || [];
    while (casings.length < 2) {
      casings.push({
        model: "",
        price_type: "",
        price: 0,
        quantity: 1,
        casing_type: casings.length === 0 ? "боковой" : "прямой",
        casing_formula: "formula1",
        casing_range: "",
        height: 0,
        width: casingSize,
                                  ...
                                  <div className="grid grid-cols-4 gap-10">
                                    ...
                                    <div>
                                      ...
                                      <Input
                                        ...
                                        title={`Calculated based on type: боковой = door height + ${casingSize}, прямой = door width + ${
                                          2 * casingSize
                                        }`}
                                        disabled={
