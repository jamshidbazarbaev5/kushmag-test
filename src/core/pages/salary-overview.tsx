...
export default function SalaryOverviewPage() {
  ...
  const filteredSalaries = monthlySalaries.filter(salary => {
    ...
    const userRole = salary.user_details?.role || 
      users.find(user => user.id === salary.user)?.role || '';
    
    const matchesSearch = userName.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         salary.user_details?.username?.toLowerCase().includes(searchTerm.toLowerCase());
        ...
        <TabsContent value="table" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                {t('navigation.monthly_salaries')} - {filteredSalaries.length} {t('forms.staff_member')}
              </CardDescription>
            </CardHeader>
            <CardContent>
              <Table>
                  ...
                  {filteredSalaries.map((salary) => {
                    ...
                    return (
                      <TableRow key={salary.id}>
                        ...
                        <TableCell>{(salary.fixed_salary || 0)}</TableCell>
                        <TableCell>{salary.order_percentage || 0}%</TableCell>
                        <TableCell>{(salary.order_percentage_salary || 0)}</TableCell>
